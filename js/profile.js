...
function renderUserDocuments() {
  ...
  if (avgAtsScore) {
    avgAtsScore.textContent = cvCount > 0 ? Math.round(totalAts / cvCount) + "%" : "--";
  }
  ...
  if (filteredDocs.length === 0) {
    var filterLabel = currentFilter === "ALL" ? "resumes or portfolios" : currentFilter + "s";
    container.innerHTML = [
        ...
        '<div class="doc-card p-3 h-100 d-flex flex-column justify-content-between">',
          ...
          '<div class="d-flex align-items-center justify-content-between pt-2 border-top gap-2">',
            '<div class="d-flex gap-2">',
              ...
              '</a>',
              (isCV
                ? '<a href="cv.html?docId=' + encodeURIComponent(doc.id) + '" target="_blank" class="btn btn-sm btn-outline-primary px-2 rounded-2" title="Open Full CV"><i class="fa-solid fa-arrow-up-right-from-square"></i></a>'
                : '<a href="portfolio.html?docId=' + encodeURIComponent(doc.id) + '" target="_blank" class="btn btn-sm btn-outline-success px-2 rounded-2" title="Open Live Portfolio"><i class="fa-solid fa-arrow-up-right-from-square"></i></a>'),
            '</div>',
...
window.promptCreateDoc = function(type) {
  ...
  var defaultTitle = type === "CV" ? "Software Engineer Resume" : "Web Developer Portfolio";
