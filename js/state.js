...
StateManager.prototype.saveState = function () {
  ...
  this._saveTimer = setTimeout(function () {
    ...
    try {
      ...
      storage.setItem(self.getStorageKey(), JSON.stringify(self.state));
      if (!self.isGuest()) {
        self.syncWithProfileList();
      }
    } catch (e) {
...
StateManager.prototype.calculateAtsScore = function () {
  ...
  if (p.fullName && p.fullName.trim() && p.fullName !== "Your Name")
    score += 10;
  if (p.professionalTitle && p.professionalTitle.trim()) score += 5;
  if (p.email && p.email.trim() && p.email !== "user@example.com") score += 5;
  if (p.phone && p.phone.trim()) score += 5;
  if (p.address && p.address.trim()) score += 5;

  if (state.summary && state.summary.trim().length > 50) {
    score += 15;
  ...
  if (state.education && state.education.length > 0) score += 10;

  if (state.skills && state.skills.length > 0) {
    score += Math.min(state.skills.length * 3, 15);
  }

  if (state.projects && state.projects.length > 0) score += 10;
...
window.addEventListener("beforeunload", function () {
  if (window.CVState && typeof window.CVState.flushSave === "function") {
    window.CVState.flushSave();
  }
});
