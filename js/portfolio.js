...
function renderTimeline(items, isEducation) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var dotClass =
      isFirst && !isEducation ? "timeline-dot" : "timeline-dot is-muted";
    ...
    html += [
      ...
      item.period
        ? '<div class="timeline-range">' + escapeHtml(item.period) + "</div>"
        : "",
...
function renderPortfolio() {
  ...
  if (!state) {
    root.innerHTML =
      '<div class="p-4 text-center text-muted">No portfolio data found.</div>';
    return;
  }
  ...
  if (links.github)
    socialLinks.push({
      href: links.github.startsWith("http")
        ? links.github
  ...
  if (links.website)
    socialLinks.push({
      href: links.website.startsWith("http")
        ? links.website
        : "https://" + links.website,
      icon: "public",
      label: "Website",
  ...
  if (links.behance)
    socialLinks.push({
      href: links.behance.startsWith("http")
        ? links.behance
        : "https://behance.net/" + links.behance,
      icon: "palette",
      label: "Behance",
    });
  ...
  for (var i = 0; i < socialLinks.length; i++) {
    ...
    socialHtml += [
      '<a class="backgroundLinks" href="' +
        escapeHtml(sLink.href) +
  ...
  for (var eIdx = 0; eIdx < exps.length; eIdx++) {
    var e = exps[eIdx];
    var expPeriod = e.currentlyWorking
      ? (e.startDate || "") + " – Present"
  ...
  for (var pIdx = 0; pIdx < projects.length; pIdx++) {
    var techList = Array.isArray(projects[pIdx].technologies)
      ...
      : projects[pIdx].technologies
        ? String(projects[pIdx].technologies).split(",")
        : [];
  ...
  if (experience.length)
    statCards.push({ value: experience.length, label: "Positions" });
  if (projects.length)
    statCards.push({ value: projects.length, label: "Projects" });
  if (skills.length) statCards.push({ value: skills.length, label: "Skills" });
  if (certifications.length)
    statCards.push({ value: certifications.length, label: "Certificates" });
  ...
  if (experience.length || education.length) {
    ...
    if (experience.length) {
      expEduHtml +=
        sectionHeader("work", "Work Experience") +
      ...
      projectCards += [
        ...
        projPeriod
          ? '<span class="small text-secondary">' +
            escapeHtml(projPeriod) +
            "</span>"
          : "",
        "</div>",
        proj.role
          ? '<div class="small text-primary fw-medium mb-2">' +
        ...
        proj.description
          ...
          : "",
        techBadges
          ? '<div class="d-flex flex-wrap gap-1 mt-2">' + techBadges + "</div>"
          : "",
        "</div>",
      ].join("");
    }
  ...
  if (skills.length || technologies.length) {
    ...
    if (skills.length) {
      skillsHtml +=
        sectionHeader("code", "Core Skills") +
    ...
    if (technologies.length) {
      ...
      for (var k = 0; k < technologies.length; k++) {
        techTags +=
          '<div class="backgroundTechnology">' +
          escapeHtml(technologies[k]) +
          "</div>";
      }
      skillsHtml +=
  ...
  if (services.length > 0) {
    ...
    for (var sv = 0; sv < services.length; sv++) {
      ...
      serviceCards += [
        '<div class="portfolio-card-item">',
        '<h4 class="h6 fw-bold text-dark mb-1"><i class="fa-solid fa-check text-primary me-2"></i>' +
  ...
  if (certifications.length || courses.length || awards.length) {
    ...
    for (var c1 = 0; c1 < certifications.length; c1++) {
      ...
      listItems += [
        ...
        "<div>",
        '<div class="fw-bold text-dark small"><i class="fa-solid fa-certificate text-warning me-2"></i>' +
          escapeHtml(crt.name) +
          "</div>",
        crt.organization
          ? '<div class="text-secondary small">' +
            escapeHtml(crt.organization) +
            "</div>"
          : "",
        "</div>",
        crt.issueDate
          ? '<span class="small text-muted">' +
            escapeHtml(crt.issueDate) +
            "</span>"
          : "",
        "</div>",
      ].join("");
    ...
    for (var c2 = 0; c2 < courses.length; c2++) {
      ...
      listItems += [
        '<div class="portfolio-card-item">',
        '<div class="fw-bold text-dark small"><i class="fa-solid fa-book-open text-primary me-2"></i>' +
          escapeHtml(crs.name) +
          "</div>",
        crs.organization
          ? '<div class="text-secondary small">' +
            escapeHtml(crs.organization) +
            "</div>"
          : "",
        "</div>",
      ].join("");
    ...
    for (var a1 = 0; a1 < awards.length; a1++) {
      ...
      listItems += [
        '<div class="portfolio-card-item">',
        '<div class="fw-bold text-dark small"><i class="fa-solid fa-trophy text-warning me-2"></i>' +
          escapeHtml(awd.name) +
          "</div>",
        awd.organization
          ? '<div class="text-secondary small">' +
            escapeHtml(awd.organization) +
            "</div>"
          : "",
        "</div>",
      ].join("");
  ...
  if (languages.length > 0) {
    ...
    for (var lg = 0; lg < languages.length; lg++) {
      ...
      langCards += [
        ...
        '<span class="fw-bold small text-dark"><i class="fa-solid fa-language text-primary me-2"></i>' +
          escapeHtml(lang.language) +
          "</span>",
        '<span class="badge bg-light text-dark border px-2 py-1 small">' +
          escapeHtml(lang.proficiency || "Proficient") +
          "</span>",
        "</div>",
      ].join("");
    }
  ...
  if (volunteer.length || organizations.length) {
    ...
    for (var v1 = 0; v1 < volunteer.length; v1++) {
      ...
      voItems += [
        '<div class="portfolio-card-item">',
        '<div class="fw-bold text-dark small"><i class="fa-solid fa-hand-holding-heart text-danger me-2"></i>' +
          escapeHtml(vl.name) +
          "</div>",
        vl.organization
          ? '<div class="text-secondary small">' +
            escapeHtml(vl.organization) +
            "</div>"
          : "",
        "</div>",
      ].join("");
    ...
    for (var o1 = 0; o1 < organizations.length; o1++) {
      ...
      voItems += [
        '<div class="portfolio-card-item">',
        '<div class="fw-bold text-dark small"><i class="fa-solid fa-sitemap text-primary me-2"></i>' +
          escapeHtml(og.name) +
          "</div>",
        og.organization
          ? '<div class="text-secondary small">' +
            escapeHtml(og.organization) +
            "</div>"
          : "",
        "</div>",
      ].join("");
  ...
  root.innerHTML = [
    ...
    '<div class="fw-bold fs-3 text-dark mb-1">' +
      ...
      "</div>",
    p.professionalTitle
      ? '<div class="fs-5 text-primary fw-medium mb-2">' +
        escapeHtml(p.professionalTitle) +
        "</div>"
      : "",
    p.address
      ? '<div class="small text-secondary mb-2"><i class="fa-solid fa-location-dot me-1"></i>' +
        escapeHtml(p.address) +
        "</div>"
      : "",
    aboutText
      ? '<p class="discriptionOfUser mb-3 small">' +
